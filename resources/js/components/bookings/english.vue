<template>
  <div class="bookings">
      ...
      <div class="col-md-12">
        <div class="card rounded-0 border-info shadow">
          <div class="card-header border-bottom-0">
            <span>
              <span class="h4"><i class="text-info fas fa-globe"></i> Reserva Ingles </span>
              ...
              <span v-if="can('bookingsschedules-view')">
                 ...
                 <b-button variant="dark" size="sm" class="float-right mx-1" @click="placement">
                  <i class="fas fa-user-check"></i> <span class="mx-1">Exonerar</span>
                 </b-button>
    ...
    <b-modal centered v-model="openModal" size="lg" ref="my-modal" header-bg-variant="dark" header-close-content="" title="Hacer una reserva" @hidden="resetModal">
      <div class="container">
        <div class="row">
          <div class="col-md-12">
            <b-form>
      ...
      <template v-slot:modal-footer>
        <div class="w-100">
          <b-button v-if="makeAppointment == 2 && booking.time != null && disableDate.schedule != null && booking.models.length <= 0" type="button" class="float-left mx-3" size="sm" variant="danger" @click.stop.prevent="banAppointment">
    ...
    <b-modal centered id="detail-modal" ref="detail" size="lg" header-bg-variant="dark" header-close-content="" title="Detalles de la agenda" @hidden="resetModal">
      <div class="container">
        <div class="row">
          <div class="col-md-12">
            <div class="row p-3">
...
<!--                <p v-if="booking.description">
      ...
      <template v-slot:modal-footer>
        <div class="w-100">
          <b-button type="button" v-if="can('english-delete') && booking.status == 0" class="float-right mx-2" size="sm" variant="danger" @click.prevent="deleteBooking(booking.id)">
            <i class="fas fa-trash"></i> Delete
          </b-button>

          <b-button v-if="can('english-edit') && booking.status == 0 && addAnother == true" type="button" class="float-right mx-2" size="sm" variant="primary" @click="addMoreModels">
            <i class="fas fa-plus"></i> Añadir mas modelos
          </b-button>

          <b-button v-if="can('english-edit') && booking.status == 0 && reschedule == true" type="button" class="float-right mx-2" size="sm" variant="dark" @click="rescheduleModal">
            <i class="fab fa-algolia"></i> Reprogramar
          </b-button>

          <b-button v-if="can('english-edit') && booking.status == 0 && bookingProccess.status != 2 && booking.startDate == moment().format('YYYY-MM-DD')" type="button" class="float-right" size="sm" variant="success"
                    @click.prevent="updateAppointment(booking.id)">
            <i class="fas fa-check"></i> Confirmar asistencia
          </b-button>

          <b-button v-if="can('english-edit') && bookingProccess.status == 2 && booking.status == 0" type="button" class="float-right" size="sm" variant="warning" @click.prevent="updateAppointment(booking.id)">
    ...
    <b-modal centered id="placement-modal" size="lg" ref="placement" hide-footer header-bg-variant="dark" header-close-content="" title="Exonerar usuarios" @show="resetModal" @hidden="resetModal">
      <div class="container">
        <div class="row">
  ...
  methods: {
    ...
    handleDateSelect(selectInfo) {
      ...
      if(currentDate < selectedDate){
        ...
        this.getSchedule()
        console.log("selectInfo.Start: " + this.booking.startDate)
      }
    ...
    removeExonerate(id){
      ...
      axios.delete(url).then((response) => {
        ...
        Toast.fire({
          icon: "success",
          title: "Usuario removido correctamente!",
        })
      }).catch((error) => {
        console.log(error)
      })
    ...
    addModels() {
      ...
      const url = route("bookings.addMoreModels", {id})
      this.isCreating = true
