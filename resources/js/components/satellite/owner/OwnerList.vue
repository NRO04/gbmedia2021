    ...
    <div class="col-lg-12">
      <b-row class="justify-content-end m-2">
        <b-col md="12">
          ...
          <span class="mx-2"
            ><i class="fas text-info fa-money-check-alt"></i> Camara y
            Comercio</span
          >
          <span class="mx-2"
            ><i class="fas text-warning fa-dumpster"></i> Composición
            Accionaria</span
          >
          <span class="mx-2"
            ><i class="fas text-danger fa-university"></i> Certificación
            Bancaria</span
          >
        </b-col>
      </b-row>
        ...
        >
          ...
          <template #cell(actions)="row">
            <a
              ...
              title="Pagos"
              target="_blank"
              :href="'payment/payroll/owner/' + row.item.id"
              ><i class="fa fa-dollar-sign"></i
            ></a>
            <a
              v-if="can('satellite-owners-view')"
              target="_blank"
              :href="'account/list/' + row.item.id"
              class="btn btn-info btn-sm"
              title="Cuentas"
              ><i class="fa fa-eye"></i
            ></a>
            <a
              v-if="can('satellite-owners-edit')"
              :href="'owner/edit/' + row.item.id"
            ...
            ></a>
            <button
              data-target="#modal-resume"
              data-toggle="modal"
              class="btn btn-info btn-sm"
              title="Resumen de Pagos"
              @click="getOwnerPayrolls(row.item.id, row.item.owner)"
            >
              <i class="fa fa-list"></i>
            </button>
            <button
              data-target="#modal-graphics"
              data-toggle="modal"
              class="btn btn-secondary btn-sm"
              title="Graficas del Pago"
              @click="graphicOwner(row.item.id, row.item.owner)"
            >
          ...
          <template #cell(documents)="row">
            <span v-if="row.item.rut == 1" title="RUT"
              ><i class="fas text-success fa-check"></i
            ></span>
            <span
              v-if="row.item.chamber_commerce == 1"
              class="mx-2"
              title="Camara y Comercio"
              ><i class="fas text-info fa-money-check-alt"></i
            ></span>
            <span
              v-if="row.item.shareholder_structure == 1"
              class="mx-2"
              title="Composición Accionaria"
              ><i class="fas text-warning fa-dumpster"></i
            ></span>
            <span
              v-if="row.item.bank_certification == 1"
              class="mx-2"
              title="Certificación Bancaria"
              ><i class="fas text-danger fa-university"></i
            ></span>
          </template>
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">
              ...
              <span class="text-danger" v-text="owner_name"></span>
            </h4>
          </div>
    ...
    <div id="modal-resume" class="modal fade" role="dialog">
      <div class="modal-dialog modal-dark modal-lg">
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">
              Resumen de pago del Propietario
              <span class="text-danger" v-text="owner_name"></span>
            </h4>
          </div>
          <div class="modal-body">
...
export default {
  ...
  methods: {
    ...
    getOwners() {
      this.show = true;
      axios
        .get(route("satellite.get_owners", { months: this.months }))
        .then((response) => {
    ...
    getAccounts(owner_id, owner) {
      ...
      axios
        .get(route("satellite.get_owner_accounts", { owner_id: owner_id }))
        .then((response) => {
    ...
    getOwnerPayrolls(owner_id, owner) {
      ...
      axios
        .get(
          route("satellite.payment.get_owner_payment_payrolls", {
            owner_id: owner_id,
          })
        )
        .then((response) => {
