<template>
  <div id="container-statistics-resume">
    <div class="row justify-content-between mb-4">
      <div class="col-md-4">
        <b-form-group>
          <VueCtkDateTimePicker :no-value-to-custom-elem="(true)" v-model="search" autoClose dark range noShortcuts noLabel noButton color="blue" inputSize="sm" format="YYYY-MM-DD" formatted="ll" id="RangeDatePicker">
            <b-form-input size="sm" v-model="search.start +' / '+ search.end"></b-form-input>
          </VueCtkDateTimePicker>
        </b-form-group>
      </div>
      <div class="col-md-4">
        <b-form-select v-model="search.selectedLocation" @change="GetAttendanceStats()" size="sm">
          <b-form-select-option v-for="(location, i) in locations" :key="i" :value="location.id">{{ location.name }}</b-form-select-option>
        </b-form-select>
      </div>
    </div>
    <div class="row">
      <div class="col-sm-6 col-lg-3">
        <div class="card text-white bg-gradient-primary container-resume-1">
          <div class="card-body card-body pb-0 d-flex justify-content-between align-items-start">
            <div>
              <div id="container-week-goal">Meta semanal estudio: <b id="studio-week-goal" class="text-bold">{{ overallData.studio_goal }}</b></div>
              <div id="container-week-earnings">
                Total alcanzado: <b id="studio-week-earnings" class="text-bold">{{ overallData.reached_amount }}</b> <i class="fa fa-info-circle"></i> <i class="fa fa-info-circle"></i>
              </div>
              <div id="container-week-hour-billing">
                Facturación por hora: <b id="studio-week-hour-billing" class="text-bold">{{ overallData.earnings_per_hour }}</b>
              </div>
              <div id="container-week-reached">
                Alcanzado: <b id="studio-week-reached" class="text-bold">22.27%</b>
              </div>
              <div id="container-week-models" :title="'Modelos activas en el estudio:' + overallData.active_models + '<br>Modelos facturando por ahora:' + overallData.models_earning" v-b-tooltip.hover.html>
                Activas: <b id="studio-week-active-models" class="text-bold">{{ overallData.active_models }}</b>
                / Facturando: <b id="studio-week-billing-models" class="text-bold">{{ overallData.models_earning }}</b>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="card-body card-body pb-0 d-flex justify-content-between align-items-start">
            <div>
              <div id="container-week-earnings1">
                Total Inasistencias: <b id="studio-week-earnings1" class="text-bold">{{ overallData.unjustified }}</b>
              </div>
              <div id="container-week-earnings2">
                <i class="fas fa-plus"></i> Inasistencias: <b id="studio-week-earnings2" class="text-bold">{{ most.nick + "("+ most.unjustified_days +")" }}</b>
              </div>
              <div id="container-week-hour-billing2">
                Perdido Inasistencias: <b id="studio-week-hour-billing2" class="text-bold">${{ (lost).toFixed(2) }}</b>
              </div>
          ...
          <div class="card-body card-body pb-0 d-flex justify-content-between align-items-start">
            <div>
              <div id="container-week-earnings3">
                <i class="fas fa-plus"></i> Ventas: <b id="studio-week-earnings3" class="text-bold">{{ best_selling.nick + "($"+ best_selling.value+")" }}</b>
              </div>
              <div id="container-week-earnings4">
                <i class="fas fa-minus"></i> Ventas: <b id="studio-week-earnings4" class="text-bold">{{ (worst_selling.nick + "($"+ worst_selling.value +")") }}</b>
              </div>
              <div id="container-week-hour-billing3">
                <i class="fas fa-plus"></i> Rendimiento <b id="studio-week-hour-billing3" class="text-bold">$ {{ (worst_selling.nick + "($"+ worst_selling.value +")") }}</b>
              </div>
              <div id="container-week-reached3">
                <i class="fas fa-minus"></i> Rendimiento: <b id="studio-week-hour-billing4" class="text-bold">$ {{ (most.goal * most.unjustified_days).toFixed(2) }}</b>
              </div>
            </div>
          </div>
        </div>
