...
use App\Http\Controllers\Settings\LocationPermissionController;
    ...
    public function getLocations(SettingLocation $locations, Request $request)
    {
        ...
        foreach ($locations as $location)
        {
            if (Auth::user()->can('location-edit')) {
                $btn_edit = "<button type='button' class='btn btn-sm btn-warning' onclick='Edit(".$location->id.")'><i class='fa fa-edit'></i></button>";
            }

            if (Auth::user()->can('location-delete')) {
                $btn_delete = "<button type='button' disabled class='btn btn-sm btn-danger' onclick='Delete(".$location->id.")'><i class='fa fa-trash'></i></button>";
            }
